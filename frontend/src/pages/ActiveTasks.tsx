...
export default function ActiveTasks() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((t) => {
        const searchable = [t.title, t.assigned_to, t.client, t.responsible, t.id.toString(), t.state]
          .filter(Boolean)
          .join(' ')
        return searchable
          .normalize('NFD')
    ...
    return filtered
  }, [activeTasks, searchTerm, assignedFilter, stateFilter, clientFilter, responsibleFilter])

  const availableAssignees = useMemo(() => {
    const set = new Set(activeTasks.map((t) => (t.assigned_to || 'Não atribuído').trim()).filter(Boolean))
    return Array.from(set).sort()
  }, [activeTasks])

  const availableStates = useMemo(() => {
    const set = new Set(activeTasks.map((t) => t.state).filter(Boolean))
    return Array.from(set).sort()
  }, [activeTasks])

  const availableClients = useMemo(() => {
    const set = new Set(activeTasks.map((t) => (t.client || '–').trim()).filter((v) => v && v !== '–'))
    return Array.from(set).sort()
  }, [activeTasks])

  const availableResponsibles = useMemo(() => {
    const set = new Set(activeTasks.map((t) => (t.responsible || '–').trim()).filter((v) => v && v !== '–'))
    return Array.from(set).sort()
  }, [activeTasks])
  ...
  const sortedTasks = useMemo(() => {
    ...
    return [...filteredTasks].sort((a, b) => {
      ...
      let aVal: string | number = key === 'days_overdue' ? a.days_overdue ?? 0 : (a as any)[key] ?? ''
      let bVal: string | number = key === 'days_overdue' ? b.days_overdue ?? 0 : (b as any)[key] ?? ''
  ...
  if (error) {
    return (
      <div className="glass dark:glass-dark p-6 rounded-lg text-center">
        <p className="text-red-500">Erro ao carregar tasks ativas</p>
      </div>
    )
  }
      ...
      <div className="glass dark:glass-dark rounded-lg overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-blue-600/20 dark:bg-blue-800/20">
              <tr>
                ...
                >
                  Estado {sortConfig?.key === 'state' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                </th>
                <th
                  className="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-blue-600/30"
                ...
                >
                  Data Alvo {sortConfig?.key === 'target_date' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                </th>
                <th
                  className="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-blue-600/30"
                  onClick={() => handleSort('days_overdue')}
                >
                  Atraso {sortConfig?.key === 'days_overdue' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {sortedTasks.map((t: Task) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{formatDate(t.target_date)}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {t.days_overdue != null && t.days_overdue > 0 ? (
                      <span className="text-red-600 dark:text-red-400 font-medium">{t.days_overdue}d</span>
                    ) : (
        ...
        {sortedTasks.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 dark:text-gray-400">Nenhuma task ativa encontrada</p>
          </div>
        )}
      </div>
    </div>
