...
const SlideDashboardChamados = () => {
  ...
  const [filtroTempoAberto, setFiltroTempoAberto] = useState<string | null>(null);
  const [filtroUltimaAtualizacao, setFiltroUltimaAtualizacao] = useState<string | null>(null);
  const [filtroResponsavel, setFiltroResponsavel] = useState<string | null>(null);
  const [filtroTicket, setFiltroTicket] = useState<string | null>(null);
  ...
  const { data: statusDataOriginal = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await chamadosApi.getDashboardStatus(filtroTicket, analistaFilter, analistasSelecionados);
  ...
  const { data: tempoAbertoDataOriginal = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await chamadosApi.getDashboardTempoAberto(filtroTicket, analistaFilter, analistasSelecionados);
      return response.data || [];
    },
  });
  ...
  const { data: ultimaAtualizacaoDataOriginal = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await chamadosApi.getDashboardUltimaAtualizacao(filtroTicket, analistaFilter, analistasSelecionados);
      return response.data || [];
    },
  });
  ...
  const { data: detalhesData = { dados: [], total: 0 } } = useQuery({
    ...
    queryFn: async () => {
      const response = await chamadosApi.getDashboardDetalhes(
        filtroStatus,
        filtroTempoAberto,
  ...
  const { data: causaRaizData = { dados: [], total: 0 } } = useQuery({
    ...
    queryFn: async () => {
      const response = await chamadosApi.getDashboardCausaRaiz(null, analistaFilter, analistasSelecionados);
      return response.data || { dados: [], total: 0 };
    },
  });
  ...
  const { data: emAndamentoData = { total: 0, dados: [] } } = useQuery({
    ...
    queryFn: async () => {
      const response = await chamadosApi.getDashboardEmAndamento(null, analistaFilter, analistasSelecionados);
      return response.data || { total: 0, dados: [] };
    },
  });

  // Função auxiliar para calcular categoria de tempo aberto
  const calcularCategoriaTempoAberto = (dataCriacao: string | null): string | null => {
  ...
  const calcularCategoriaUltimaAtualizacao = (dataAlteracao: string | null): string | null => {
    if (!dataAlteracao) return null;
    const dias = Math.floor((new Date().getTime() - new Date(dataAlteracao).getTime()) / (1000 * 60 * 60 * 24));
    if (dias <= 1) return 'Até 1 dia';
    if (dias >= 2 && dias <= 5) return 'De 2 a 5 dias';
    if (dias >= 6 && dias <= 10) return 'De 6 a 10 dias';
    if (dias >= 11 && dias <= 20) return 'De 11 a 20 dias';
  ...
  const statusDataFiltrado = useMemo(() => {
    ...
    if (filtroTicket && detalhesTicketData.dados?.length > 0) {
      ...
      detalhesTicketData.dados?.forEach((item: any) => {
        if (item.status) {
          statusCount[item.status] = (statusCount[item.status] || 0) + 1;
        }
      });

      return statusDataOriginal.map((item: any) => ({
        ...item,
    ...
    if (!filtroTempoAberto && !filtroUltimaAtualizacao && !filtroResponsavel) {
      if (filtroStatus) {
        return statusDataOriginal.filter((item: any) => item.status === filtroStatus);
      }
    ...
    const statusCount: Record<string, number> = {};
    detalhesData.dados?.forEach((item: any) => {
      if (item.status) {
        statusCount[item.status] = (statusCount[item.status] || 0) + 1;
      }
    });
  ...
  const tempoAbertoDataFiltrado = useMemo(() => {
    ...
    if (filtroTicket && detalhesTicketData.dados?.length > 0) {
      ...
      return tempoAbertoDataOriginal.map((item: any) => ({
        ...item,
  ...
  const ultimaAtualizacaoDataFiltrado = useMemo(() => {
    ...
    if (filtroTicket && detalhesTicketData.dados?.length > 0) {
      ...
      return ultimaAtualizacaoDataOriginal.map((item: any) => ({
        ...item,
        quantidade: categoriaCount[item.categoria] || 0
      }));
    }
  ...
  const causaRaizDataFiltrado = useMemo(() => {
    ...
    const ticketsFiltrados = new Set(detalhesData.dados?.map((item: any) => item.ticket) || []);
    const dadosFiltrados = causaRaizData.dados?.filter((item: any) => ticketsFiltrados.has(item.ticket)) || [];
  ...
  const emAndamentoDataFiltrado = useMemo(() => {
    ...
    // Recalcular baseado nos detalhes quando há filtros (incluindo ticket)
    const ticketsFiltrados = new Set(detalhesData.dados?.map((item: any) => item.ticket) || []);
    const dadosFiltrados = emAndamentoData.dados?.filter((item: any) => ticketsFiltrados.has(item.ticket)) || [];
  ...
  const getCorSemafaro = (cor: string | null): string => {
    switch (cor) {
      case 'verde':
        return 'bg-green-500';
      case 'amarelo':
  ...
  const handleClickTempoAberto = (categoria: string) => {
  ...
  const handleClickUltimaAtualizacao = (categoria: string) => {
  ...
  const handleClickResponsavel = (responsavel: string) => {
  ...
  return (
    <div className="w-full h-full p-2 bg-gray-50 dark:bg-gray-900 flex flex-col">
      <div className="grid grid-cols-12 gap-2 flex-1 min-h-0">
        ...
        <div className="col-span-9 flex flex-col min-h-0">
          ...
          <div className="grid grid-cols-3 gap-2 flex-shrink-0 mb-4" style={{ height: '350px' }}>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-2 flex flex-col" style={{ height: '350px', maxHeight: '350px' }}>
              <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 mb-1">Chamados por Status</h3>
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-white dark:bg-gray-800 z-10">
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        <th className="text-left py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Status</th>
                        <th className="text-right py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Qtd</th>
                      </tr>
                    </thead>
                    <tbody>
                      {statusDataFiltrado.map((item: any, index: number) => (
                        <tr
                          key={index}
                          className={`border-b border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors ${filtroStatus === item.status ? 'bg-blue-100 dark:bg-blue-800' : ''
                            }`}
                          onClick={() => handleClickStatus(item.status)}
                        >
                          <td className="py-1 px-1 text-gray-800 dark:text-gray-200 text-sm">{item.status}</td>
                          <td className="py-1 px-1 text-right font-semibold text-gray-800 dark:text-gray-200 text-sm">
                            {item.quantidade}
                          </td>
                        </tr>
                      ))}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-2 flex flex-col" style={{ height: '350px', maxHeight: '350px' }}>
              <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 mb-1">Tempo de chamados (Aberto)</h3>
              <div className="overflow-y-auto flex-1 min-h-0 scrollbar-thin-hover" style={{ maxHeight: 'calc(350px - 50px)' }}>
                {tempoAbertoDataFiltrado.length > 0 ? (
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-white dark:bg-gray-800 z-10">
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        <th className="text-left py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Criado</th>
                        <th className="text-center py-1 px-1 font-semibold text-gray-700 dark:text-gray-300 w-2"></th>
                        <th className="text-right py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Qtd</th>
                      </tr>
                    </thead>
                    <tbody>
                      {tempoAbertoDataFiltrado.map((item: any, index: number) => (
                        <tr
                          key={index}
                          className={`border-b border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors ${filtroTempoAberto === item.categoria ? 'bg-blue-100 dark:bg-blue-800' : ''
                            }`}
                          onClick={() => handleClickTempoAberto(item.categoria)}
                        >
                          <td className="py-1 px-1 text-gray-800 dark:text-gray-200 text-sm">{item.categoria}</td>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-2 flex flex-col" style={{ height: '350px', maxHeight: '350px' }}>
              <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 mb-1">Última atualização</h3>
              <div className="overflow-y-auto flex-1 min-h-0 scrollbar-thin-hover" style={{ maxHeight: 'calc(350px - 50px)' }}>
                {ultimaAtualizacaoDataFiltrado.length > 0 ? (
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-white dark:bg-gray-800 z-10">
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        <th className="text-left py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Atualização</th>
                        <th className="text-center py-1 px-1 font-semibold text-gray-700 dark:text-gray-300 w-2"></th>
                        <th className="text-right py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Qtd</th>
                      </tr>
                    </thead>
                    <tbody>
                      {ultimaAtualizacaoDataFiltrado.map((item: any, index: number) => (
                        <tr
                          key={index}
                          className={`border-b border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors ${filtroUltimaAtualizacao === item.categoria ? 'bg-blue-100 dark:bg-blue-800' : ''
                            }`}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-2 flex flex-col" style={{ height: '350px', maxHeight: '350px' }}>
            <div className="flex justify-between items-center mb-1 flex-shrink-0">
              <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200">Detalhes dos Chamados</h3>
              {temFiltros && (
                <button
                  onClick={limparFiltros}
                  className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-semibold px-2 py-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900"
                >
                  Limpar Filtros
                </button>
              )}
              ...
              {detalhesData.dados?.length > 0 ? (
                <table className="w-full text-sm">
                  <thead className="bg-gray-100 dark:bg-gray-700 sticky top-0 z-10">
                    <tr>
                      <th className="text-left py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Ticket</th>
                      <th className="text-left py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Responsável</th>
                      <th className="text-left py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Data Criação</th>
                      <th className="text-left py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Data Alteração</th>
                      <th className="text-center py-1 px-1 font-semibold text-gray-700 dark:text-gray-300">Atualização</th>
                    </tr>
                  </thead>
                  <tbody>
                    {detalhesData.dados?.map((item: any, index: number) => (
                      <tr
                        key={index}
                        className={`border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer ${filtroTicket === item.ticket ? 'bg-blue-100 dark:bg-blue-800' : ''
                          } ${filtroResponsavel === item.responsavel ? 'bg-blue-50 dark:bg-blue-900' : ''}`}
                      >
                        <td
                          className="py-1 px-1 text-gray-800 dark:text-gray-200"
                        ...
                        <td className="py-1 px-1 text-gray-800 dark:text-gray-200">{formatarData(item.data_criacao)}</td>
                        <td className="py-1 px-1 text-gray-800 dark:text-gray-200">{formatarData(item.data_alteracao)}</td>
                        <td className="py-1 px-1 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <div
                              className={`w-2 h-2 rounded-full ${getCorSemafaro(item.cor_atualizacao)}`}
                            ></div>
                            <span className="text-gray-800 dark:text-gray-200">{item.atualizacao}</span>
                          </div>
                        </td>
                      </tr>
                    ))}
                  ...
                  <tfoot className="bg-gray-100 dark:bg-gray-700 sticky bottom-0 z-10">
                    <tr>
                      <td colSpan={4} className="py-1 px-1 font-bold text-gray-800 dark:text-gray-200">
                        Total
                      </td>
                      <td className="py-1 px-1 text-center font-bold text-gray-800 dark:text-gray-200">
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-1 overflow-y-auto scrollbar-thin-hover" style={{ maxHeight: 'calc(350px - 80px)' }}>
              {causaRaizDataFiltrado.dados?.length > 0 ? (
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {causaRaizDataFiltrado.dados?.map((item: any, index: number) => (
                      <tr
                        key={index}
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-1 overflow-y-auto scrollbar-thin-hover" style={{ maxHeight: 'calc(350px - 80px)' }}>
              {emAndamentoDataFiltrado.dados?.length > 0 ? (
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {emAndamentoDataFiltrado.dados?.map((item: any, index: number) => (
                      <tr
                        key={index}
                        className={`border-b border-white border-opacity-20 cursor-pointer hover:bg-white hover:bg-opacity-30 transition-colors ${filtroTicket === item.ticket ? 'bg-white bg-opacity-30' : ''
                          }`}
