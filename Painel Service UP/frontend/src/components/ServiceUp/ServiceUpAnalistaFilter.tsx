...
import { useAnalistaFilterContext } from '../../contexts/ServiceUpAnalistaFilterContext.jsx';

const ServiceUpAnalistaFilter = () => {
  const {
    analistaFilter,
    setAnalistaFilter,
        ...
        {showMenu && (
          <>
            ...
            <motion.div
              ...
              className="absolute left-0 top-full mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 z-[9999] max-h-96 overflow-y-auto"
            >
              <div className="p-2">
                {/* Opção Todos */}
                <button
                ...
                {analistasNaoQualiIT.length > 0 && (
                  <div className="px-2 py-1">
                    <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">Analistas Individuais:</div>
                    {analistasNaoQualiIT.map((analista) => (
                      <label
                        key={analista}
                        className="flex items-center px-2 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer"
                      >
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className="text-sm text-gray-700 dark:text-gray-300">{analista}</span>
                      </label>
                    ))}
                  </div>
                )}
                ...
                {analistasQualiITOrdenados.length > 0 && (
                  <>
                    <div className="h-px bg-gray-200 dark:bg-gray-600 my-2"></div>
                    <div className="px-2 py-1">
                      <div className="text-xs font-semibold text-purple-600 dark:text-purple-400 mb-2">QualiIT:</div>
...
export default ServiceUpAnalistaFilter;
