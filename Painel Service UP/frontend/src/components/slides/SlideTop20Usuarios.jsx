...
const SlideTop20Usuarios = () => {
    ...
    const { data = [], isLoading, isError, error } = useQuery({
        ...
        queryFn: async () => {
            try {
                const response = await chamadosService.getTop20Usuarios(month, year, startDate, endDate);
    ...
    const total = safeData.length > 0
        ? safeData.reduce((sum, item) => sum + (Number(item.quantidade) || 0), 0)
        : 0;

    const valores = safeData.map(item => Number(item.quantidade) || 0);
    ...
    const chartData = {
        labels: safeData.length > 0
            ? safeData.map(item => String(item.usuario || 'Sem Usuário'))
            : ['Sem dados'],
        datasets: [
            {
                label: 'Chamados',
                data: safeData.length > 0
                    ? safeData.map(item => Number(item.quantidade) || 0)
                    : [0],
    ...
    const chartOptions = {
        ...
        plugins: {
            ...
            tooltip: {
                ...
                caretSize: 8,
                callbacks: {
                    label: (context) => `Chamados: ${context.parsed.x}`
                }
            }
        },
        scales: {
