...
const SlideChamadosAtendidos = () => {
  ...
  const { data: annualData = [], isLoading: isLoadingAnnual, isError: isErrorAnnual } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await chamadosService.getAtendidos(null, selectedYear, yearStartDate, yearEndDate, analistaFilter, analistasSelecionados);
  ...
  // O formato do mes é "Nov/24", então precisamos encontrar o mês correspondente
  const getMesFiltrado = () => {
    ...
    if (filterType === 'month') {
      ...
      const mesEncontrado = safeAnnualData.find(item => item.mes === mesProcurado);
      return mesEncontrado || { chamados: 0, n1: 0, n2: 0, mes: mesProcurado };
    }

    // CASO 2: Filtro por ano (somar todos os meses)
    if (filterType === 'year') {
      const totalAnual = safeAnnualData.reduce((acc, item) => {
        return {
    ...
    queryFn: async () => {
      ...
      try {
        if (periodoAnterior.type === 'month') {
          ...
          const response = await chamadosService.getAtendidos(null, periodoAnterior.year, prevYearStartDate, prevYearEndDate, analistaFilter, analistasSelecionados);
          const data = response.data || [];
          const mesesAbrev = ['', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
          const mesAbrev = mesesAbrev[periodoAnterior.month];
          const anoAbrev = String(periodoAnterior.year).slice(-2);
          const mesProcurado = `${mesAbrev}/${anoAbrev}`;
          const mesEncontrado = data.find(item => item.mes === mesProcurado);
  ...
  const calcularDiferenca = () => {
    ...
    if (filterType === 'month') {
      ...
      const totalAnoAnterior = previousPeriodData.reduce((acc, item) => {
  ...
  if (filterType === 'month') {
    ...
    mesAnoDisplay = mesFiltrado.mes || (month && month !== 0 && month !== null && year ? `${String(month).padStart(2, '0')}/${year}` : (year ? `${year}` : ''));
  }

  // Função para identificar quais meses estão filtrados
  const getMesesFiltrados = () => {
    const mesesFiltrados = new Set();
    const mesesAbrev = ['', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  ...
  const valores = safeAnnualData.map(item => Number(item.chamados) || 0).filter(v => v != null);
  ...
  const chartData = {
    labels: safeAnnualData.length > 0
      ? safeAnnualData.map(item => String(item.mes || ''))
      : ['Sem dados'],
    datasets: [
      {
        label: 'Chamados',
        data: safeAnnualData.length > 0
          ? safeAnnualData.map(item => Number(item.chamados) || 0)
          : [0],
        borderColor: '#6366f1',
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        caretSize: 8,
        callbacks: {
          label: (context) => `Chamados: ${context.parsed.y}`
        }
      },
      datalabels: {
        display: true,
  ...
  return (
    <div className="w-full">
      <div className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-xl p-5 mb-5 shadow-lg">
        ...
        <p className="text-4xl font-bold text-white text-center">{Number(mesFiltrado.chamados) || 0}</p>
      </div>

      <div className="grid grid-cols-2 gap-5 mb-5">
        <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg p-5 shadow-md border-2 border-indigo-300">
          <h3 className="text-base font-bold text-indigo-700 text-center mb-2">N1</h3>
          <p className="text-3xl font-bold text-indigo-900 text-center">{Number(mesFiltrado.n1) || 0}</p>
        </div>
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-5 shadow-md border-2 border-purple-300">
          <h3 className="text-base font-bold text-purple-700 text-center mb-2">N2</h3>
          <p className="text-3xl font-bold text-purple-900 text-center">{Number(mesFiltrado.n2) || 0}</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-5">
        <div className="col-span-2 bg-white rounded-lg p-5 border border-gray-200 shadow-sm">
          <div className="h-64">
            <Line data={chartData} options={chartOptions} />
          </div>
        </div>

        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg p-5 border-2 border-indigo-200 shadow-lg flex flex-col items-center justify-center">
          <div className="bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full p-5 mb-4 shadow-lg">
            <i className="fas fa-chart-line text-white text-3xl"></i>
          </div>
          <p className="text-sm font-bold text-indigo-800 text-center leading-tight">
