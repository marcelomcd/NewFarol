...
const SlideAbertoFechado = () => {
  ...
  const { data = [], isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await chamadosApi.getAbertoFechado(monthToUse, yearToUse, startDateToUse, endDateToUse, analistaFilter, analistasSelecionados);
  ...
  const totalAnual = safeData.reduce((acc, item: any) => {
    return {
  ...
  const chartData = {
    labels: safeData.length > 0
      ? safeData.map((item: any) => formatMonthLabel(String(item.mes || '')))
      : ['Sem dados'],
    datasets: [
      {
        label: 'Fechado',
        data: safeData.length > 0
          ? safeData.map((item: any) => Number(item.fechado) || 0)
          : [0],
      ...
      },
      {
        label: 'Aberto',
        data: safeData.length > 0
          ? safeData.map((item: any) => Number(item.aberto) || 0)
          : [0],
        backgroundColor: (context: any) => {
          const ctx = context.chart.ctx;
          const gradient = ctx.createLinearGradient(0, 0, 0, 400);
