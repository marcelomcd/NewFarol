...
const SlideDatasul = () => {
  ...
  const { data = [], isLoading, isError, error } = useQuery({
    queryKey: ['chamados', 'datasul', month, year, startDate, endDate, analistaFilter, analistasSelecionados],
    queryFn: async () => {
      try {
        const response = await chamadosService.getDatasul(month, year, startDate, endDate, analistaFilter, analistasSelecionados);
  ...
  return (
    <div className="w-full">
      <div className="bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 rounded-2xl p-5 mb-5 shadow-lg">
        <h2 className="text-xl font-bold text-white text-center mb-2">
          Classificação por Serviço – Datasul
        </h2>
